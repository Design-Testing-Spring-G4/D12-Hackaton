
package services;

import java.util.Collection;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.test.context.ContextConfiguration;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;
import org.springframework.transaction.annotation.Transactional;
import org.springframework.util.Assert;

import utilities.AbstractTest;
import domain.Tag;

@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(locations = {
	"classpath:spring/junit.xml"
})
@Transactional
public class TagServiceTest extends AbstractTest {

	//Service under test

	@Autowired
	private TagService	tagService;


	//Test template

	protected void Template(final String username, final String name, final String name2, final Class<?> expected) {
		Class<?> caught = null;

		try {
			this.authenticate(username);

			//Creation
			final Tag tag = this.tagService.create();
			tag.setName(name);
			final Tag saved = this.tagService.save(tag);

			//Listing
			final Collection<Tag> cl = this.tagService.findAll();
			Assert.isTrue(cl.contains(saved));
			Assert.notNull(this.tagService.findOne(saved.getId()));

			//Edition
			saved.setName(name2);
			final Tag saved2 = this.tagService.save(saved);

			//Deletion
			this.tagService.delete(saved2);
			Assert.isNull(this.tagService.findOne(saved2.getId()));

			this.unauthenticate();

		} catch (final Throwable oops) {
			caught = oops.getClass();

		}

		this.checkExceptions(expected, caught);
	}

	//Driver for multiple tests under the same template.

	@Test
	public void Driver() {

		final Object testingData[][] = {

			//Test #01: Correct execution of test. Expected true.
			{
				"admin", "testName", "editName", null
			},

			//Test #02: Attempt to edit a tag without name. Expected false.
			{
				"admin", "testName", null, NullPointerException.class
			},

			//Test #03: Attempt to create a tag without name. Expected false.
			{
				"admin", null, "editName", NullPointerException.class
			}

		};

		for (int i = 0; i < testingData.length; i++)
			this.Template((String) testingData[i][0], (String) testingData[i][1], (String) testingData[i][2], (Class<?>) testingData[i][3]);
	}
}
