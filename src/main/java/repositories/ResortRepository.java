...
public interface ResortRepository extends JpaRepository<Resort, Integer> {
	...
	@Query("select r from Resort r where r.reservations.size >= (select avg(r.reservations.size)*1.15 from Resort r) order by r.reservations.size")
	Collection<Resort> resortsWithAboveAverageReservations();

	//The ratio of full resorts.
	@Query("select (select count(r) from Resort r where r.spots = 0)*1.0/count(r) from Resort r")
